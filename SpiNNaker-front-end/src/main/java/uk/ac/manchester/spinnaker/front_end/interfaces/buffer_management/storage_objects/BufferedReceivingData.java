 ...
 *       technique from the SpiNNaker system.
 *
 * The data kept includes the last
 ...
 * Python Version</a> * @author Christian-B
 */
public class BufferedReceivingData {
    ...
    private static final int DEAULT_SEQUENCE_NUMBER = 0xFF;
    /**
     * Stores the information received through the buffering output
     *       technique from the SpiNNaker system.
     *
    ...
    public BufferedReceivingData(String databasePath) {
        ...
        sequenceNo = new DefaultMap<>(DEAULT_SEQUENCE_NUMBER);
     ...
     *
     * @param location Location to check retrieved from.
     ...
     *
     * @param location The core retrieved from.
     * @param lastSequenceNumber he last sequence number
     */
    public void storeEndBufferingSequenceNumber(
            CoreLocation location, Integer lastSequenceNumber) {
     ...
     */
    public void storeEndBufferingSequenceNumber(
            HasCoreLocation location, Integer lastSequenceNumber) {
        storeEndBufferingSequenceNumber(
                location.asCoreLocation(), lastSequenceNumber);
    }
     ...
     *
     * @param location The X, Y, P and Region
     ...
     * @throws StorageException If there is a problem storing the data.
     */
    public void storeDataInRegionBuffer(
            RegionLocation location, ByteBuffer data) throws StorageException {
        storage.appendRegionContents(
                location.asCoreLocation(), location.region, data);
    }

    /**
     * Store flushed data from a region of a core on a chip,
     *      and mark it as being flushed.
     * @param location  The X, Y, P and Region
     * @param data data to be stored
     * @throws StorageException If there is a problem storing the data.
     */
