...
public class DataReceiver extends BoardLocalSupport {
    ...
    private void getDataForPlacement(Placement placement, int recordingRegionId)
        ...
        if (!receivedData.isRecordingRegionsStored(coreLocation)) {
            RecordingRegion[] regions = generateRecordingRegionsFromMachine(
                    txrx, placement, recordingDataAddress);
    ...
    /**
     * Reads recording regions from the machine.
     *
     * @param txrx Transceiver to read with
     * @param placement The core to read the data from
     * @param recordingDataAddress The address to read the data from
     * @return An array of recording regions read from the machine
     * @throws IOException If the reading goes wrong
     * @throws ProcessException If the data in the read goes wrong
     */
    static RecordingRegion[] generateRecordingRegionsFromMachine(
