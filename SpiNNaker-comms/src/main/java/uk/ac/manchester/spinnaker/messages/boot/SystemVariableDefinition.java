...
public enum SystemVariableDefinition {
	...
	router_time_phase_timer(DataType.INT, 0x20),
	/** The CPU clock frequency in MHz */
	cpu_clock_mhz(DataType.SHORT, 0x24, 200),
	/** The SDRAM clock frequency in MHz */
	sdram_clock_frequency_mhz(DataType.SHORT, 0x26, 130),
	/** Nearest-Neighbour forward parameter */
	nearest_neighbour_forward(DataType.BYTE, 0x28, 0x3F),
	/** Nearest-Neighbour retry parameter */
	nearest_neighbour_retry(DataType.BYTE, 0x29),
	/** The link peek/poke timeout in microseconds */
	link_peek_timeout_microseconds(DataType.BYTE, 0x2a, 100),
	/** The LED half-period in 10 ms units, or 1 to show load */
	led_half_period_10_ms(DataType.BYTE, 0x2b, 1),
	/**
	 * The time to wait after last BC during network initialisation in 10 ms
	 * units
	 */
	netinit_bc_wait_time(DataType.BYTE, 0x2c, 50),
	/** The phase of boot process (see enum netinit_phase_e) */
	netinit_phase(DataType.BYTE, 0x2d),
	/** The y-coordinate of the chip from which the system was booted */
	p2p_root_y(DataType.BYTE, 0x2e),
	/** The x-coordinate of the chip from which the system was booted */
	p2p_root_x(DataType.BYTE, 0x2f),
	/** The first part of the LED definitions */
	led_0(DataType.INT, 0x30, 0x1),
	/** The last part of the LED definitions */
	led_1(DataType.INT, 0x34),
	/** A word of padding */
	padding_1(DataType.INT, 0x38),
	/** The random seed */
	random_seed(DataType.INT, 0x3c),
	/** Indicates if this is the root chip */
	is_root_chip(DataType.BYTE, 0x40),
	/** The number of shared message buffers */
	n_shared_message_buffers(DataType.BYTE, 0x41, 7),
	/** The delay between nearest-neighbour packets in microseconds */
	nearest_neighbour_delay_us(DataType.BYTE, 0x42, 20),
	/** The number of watch dog timeouts before an error is raised */
	software_watchdog_count(DataType.BYTE, 0x43),
	/** A word of padding */
	padding_2(DataType.INT, 0x44),
	/** The base address of the system SDRAM heap */
	system_ram_heap_address(DataType.INT, 0x48, 1024),
	/** The base address of the user SDRAM heap */
	sdram_heap_address(DataType.INT, 0x4c, 0),
	/** The size of the iobuf buffer in bytes */
	iobuf_size(DataType.INT, 0x50, 16384),
	/** The size of the system SDRAM in bytes */
	system_sdram_bytes(DataType.INT, 0x54, 8388608),
	/** The size of the system buffer in words */
	system_buffer_words(DataType.INT, 0x58, 32768),
	/** The boot signature */
	boot_signature(DataType.INT, 0x5c),
	/** The memory pointer for nearest neighbour global operations */
	nearest_neighbour_memory_pointer(DataType.INT, 0x60),
	/** The lock */
	lock(DataType.BYTE, 0x64),
	/** Bit mask (6 bits) of links enabled */
	links_available(DataType.BYTE, 0x65, 0x3f),
	/** Last ID used in BIFF packet */
	last_biff_id(DataType.BYTE, 0x66),
	/** Board testing flags */
	board_test_flags(DataType.BYTE, 0x67),
	/** Pointer to the first free shared message buffer */
	shared_message_first_free_address(DataType.INT, 0x68),
	/** The number of shared message buffers in use */
	shared_message_count_in_use(DataType.SHORT, 0x6c),
	/** The maximum number of shared message buffers used */
	shared_message_maximum_used(DataType.SHORT, 0x6e),
	/** The first user variable */
	user_temp_0(DataType.INT, 0x70),
	/** The second user variable */
	user_temp_1(DataType.INT, 0x74),
	/** The third user variable */
	user_temp_2(DataType.INT, 0x78),
	/** The fourth user variable */
	user_temp_4(DataType.INT, 0x7c),
	/** The status map set during SCAMP boot */
	status_map(DataType.BYTE_ARRAY, 0x80, new byte[20]),
	/** The physical core ID to virtual core ID map */
	physical_to_virtual_core_map(DataType.BYTE_ARRAY, 0x94, new byte[20]),
	/** The virtual core ID to physical core ID map */
	virtual_to_physical_core_map(DataType.BYTE_ARRAY, 0xa8, new byte[20]),
	/** The number of working cores */
	n_working_cores(DataType.BYTE, 0xbc),
	/** The number of SCAMP working cores */
	n_scamp_working_cores(DataType.BYTE, 0xbd),
	/** A short of padding */
	padding_3(DataType.SHORT, 0xbe),
	/** The base address of SDRAM */
	sdram_base_address(DataType.INT, 0xc0),
	/** The base address of System RAM */
	system_ram_base_address(DataType.INT, 0xc4),
	/** The base address of System SDRAM */
	system_sdram_base_address(DataType.INT, 0xc8),
	/** The base address of the CPU information blocks */
	cpu_information_base_address(DataType.INT, 0xcc),
	/** The base address of the system SDRAM heap */
	system_sdram_heap_address(DataType.INT, 0xd0),
	/** The address of the copy of the routing tables */
	router_table_copy_address(DataType.INT, 0xd4),
	/** The address of the peer-to-peer hop tables */
	peer_to_peer_hop_table_address(DataType.INT, 0xd8),
	/** The address of the allocated tag table */
	allocated_tag_table_address(DataType.INT, 0xdc),
	/** The ID of the first free router entry */
	first_free_router_entry(DataType.SHORT, 0xe0),
	/** The number of active peer-to-peer addresses */
	n_active_peer_to_peer_addresses(DataType.SHORT, 0xe2),
	/** The address of the application data table */
	app_data_table_address(DataType.INT, 0xe4),
	/** The address of the shared message buffers */
	shared_message_buffer_address(DataType.INT, 0xe8),
	/** The monitor incoming mailbox flags */
	monitor_mailbox_flags(DataType.INT, 0xec),
	/** The IP address of the chip */
	ethernet_ip_address(DataType.BYTE_ARRAY, 0xf0, new byte[4]),
	/** A (virtual) copy of the router FR register */
	fixed_route_copy(DataType.INT, 0xf4),
	/** A pointer to the board information structure */
	board_info(DataType.INT, 0xf8),
	/** A word of padding */
	padding_4(DataType.INT, 0xfc);
