 ...
 * but with a backup of canned replies if the connection fails/times out.
 *
 * @author Christian
 */
    ...
    void testListJobs() throws IOException, SpallocServerException, Exception {
        try (AutoCloseable c = client.withConnection()) {
            ...
            if (client.isActual()) {
                ...
                jobs.forEach(d -> assertThat("Jobid > 0",
                		d.getJobID(), greaterThan(0)));
        ...
        try (AutoCloseable c = client.withConnection()) {
            ...
            if (client.isActual()) {
                ...
                List<String> foundNames =
                		machines.stream().map(Machine::getName).collect(
                				Collectors.toList());
